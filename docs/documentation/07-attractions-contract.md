...
| Price recheck | POST `/b2b/resellers/client/attraction/single/price/:activityId` body `{ hourCount, activityType, base, adultCount, childCount, infantCount: 0, date }` | old app always sends `infantCount: 0` (duplicate-key quirk — preserved); debounced; response replaces the activity (updateActivity) |
